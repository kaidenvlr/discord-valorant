...
class Notify(commands.Cog):
    ...
    async def get_endpoint_and_data(self, user_id: int) -> Tuple[API_ENDPOINT, Any]:
        data = await self.db.is_data(user_id, 'ru-RU')
    ...
    async def send_notify(self) -> None:
        ...
        for user_id in notify_users:
            try:
                ...
                # get guild language
                guild_locale = 'ru-RU'
                get_guild_locale = [guild.preferred_locale for guild in self.bot.guilds if
                                    channel_send in guild.channels]
                ...
                if data['notify_mode'] == 'Specified':
                    ...
                    for noti in user_skin_list:
                        if noti['uuid'] in skin_notify_list:
                            ...
                            view.message = await channel_send.send(content=f'||{author.mention}||', embed=embed,
                                                                   view=view)
    ...
    @tasks.loop(time=time(hour=0, minute=0, second=10))  # utc 00:00:10
